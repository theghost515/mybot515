...
app.get('/', requireAuth, (req, res) => {
  ...
  const counts = {
    ...
    videos:
      db.prepare('SELECT COUNT(*) c FROM quran_videos')
      .get().c,

    duas:
      db.prepare('SELECT COUNT(*) c FROM duas')
      .get().c,

    adhkar:
      db.prepare('SELECT COUNT(*) c FROM adhkar')
      .get().c,

    hadiths:
      db.prepare('SELECT COUNT(*) c FROM hadiths')
      .get().c,

    quotes:
      db.prepare('SELECT COUNT(*) c FROM quotes')
      .get().c,

    tickets:
      db.prepare(
        "SELECT COUNT(*) c FROM tickets WHERE status='مفتوحة'"
      )
      .get().c
  };
...
const videos =
db.prepare(
'SELECT * FROM quran_videos ORDER BY id DESC'
).all();
...
db.prepare(`
INSERT INTO quran_videos
(title,surah,reciter,youtube_url,added_by)
VALUES(?,?,?,?,?)
`)
.run(
title,
...
db.prepare(
'DELETE FROM quran_videos WHERE id=?'
)
.run(req.params.id);
...
{key:'arabic_text',label:'النص',textarea:true},
{key:'source',label:'المصدر'},
{key:'repeat_count',label:'التكرار'}
]
},
...
db.prepare(
"UPDATE tickets SET status='مغلقة' WHERE id=?"
)
.run(req.params.id);
...
console.log(
`✅ لوحة التحكم تعمل على port ${PORT}`
);
